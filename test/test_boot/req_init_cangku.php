...
define('ROOT_PATH',substr(__FILE__,0,strlen(__FILE__)-24)); //应用根目录  rtrim 'boot/req_init_cangku.php'
...
class RequestContext {
	...
	function get_path_grp_act($action,&$path,&$grp,&$act){
		...
		if(strpos($action, 'efast') === 0) {
			...
			//$action = preg_replace('/[^a-z0-9_\/]+/i', '', $action);
			exit(json_encode(array('resp_data'=>array('code'=>501, 'msg'=>'方法名必须以efast打头'))));
		}
...
function require_moudle($clazzs){
	...
	foreach ($clazzlist as $clazz)
	{
		$clazz=trim($clazz);
		$clsid=strtolower($clazz);
		if(in_array($clsid,$GLOBALS['_require_moudle_list'])) continue;
